...
import { 
  ...
  BookOpen, Calendar, Settings as SettingsIcon, ShieldAlert, Star, Filter, CalendarDays
} from 'lucide-react';
...
const App: React.FC = () => {
  ...
  // إعدادات السنة الدراسية العالمية
  ...
  if (loading) return (
    <div className="h-screen flex flex-col items-center justify-center bg-white">
      <div className="w-24 h-24 border-8 border-indigo-100 border-t-indigo-600 rounded-[2rem] animate-spin mb-8"></div>
      <p className="font-black text-indigo-600 animate-pulse">جاري تحضير البيئة الماسية...</p>
    </div>
  );
      ...
      <div className="min-h-screen bg-[#f8fafc] flex flex-col lg:flex-row font-['Cairo'] text-right" dir="rtl">
        ...
        <aside className="hidden lg:flex flex-col w-80 bg-white border-l border-slate-100 h-screen sticky top-0 z-50 shadow-2xl">
          <div className="p-12 border-b border-slate-50 flex items-center gap-4">
            <div className="bg-indigo-600 p-4 rounded-2xl text-white shadow-xl rotate-3"><Star size={24} fill="white" /></div>
            <div>
          ...
          <div className="p-8 bg-slate-50 mx-4 my-6 rounded-[2rem] space-y-3">
             <div className="flex items-center gap-2 text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">
                <CalendarDays size={14} /> الفترة الدراسية
             </div>
             <select value={activeYear} onChange={e => setActiveYear(e.target.value)} className="w-full p-3 bg-white rounded-xl font-black text-xs border-none shadow-sm outline-none text-slate-700">
                <option value="2024-2025">2024-2025</option>
                <option value="2025-2026">2025-2026</option>
             </select>
             <div className="flex bg-white p-1 rounded-xl shadow-sm">
                <button onClick={() => setActiveSemester('1')} className={`flex-1 py-2 rounded-lg font-black text-[10px] transition-all ${activeSemester === '1' ? 'bg-indigo-600 text-white' : 'text-slate-400'}`}>فصل 1</button>
                <button onClick={() => setActiveSemester('2')} className={`flex-1 py-2 rounded-lg font-black text-[10px] transition-all ${activeSemester === '2' ? 'bg-indigo-600 text-white' : 'text-slate-400'}`}>فصل 2</button>
             </div>
          </div>
          ...
          <div className="p-8 border-t border-slate-50 space-y-4">
             <NavLink to="/settings" className="flex items-center gap-4 text-slate-400 font-black px-8 py-4 hover:bg-slate-50 rounded-[1.5rem] transition-all"><SettingsIcon size={20} /> الإعدادات</NavLink>
        ...
        <main className="flex-1 min-w-0 flex flex-col">
          <header className="px-8 md:px-14 h-24 flex items-center justify-between bg-white/60 backdrop-blur-2xl border-b border-slate-50 sticky top-0 z-40">
             <div className="flex items-center gap-6">
                ...
                <div>
                   <p className="text-[10px] font-black text-indigo-600 uppercase tracking-widest">{isAdmin ? 'المدير العام' : 'المعلم المعتمد'}</p>
                   <h2 className="text-xl font-black text-slate-900">{profile.full_name}</h2>
                </div>
             </div>
             <div className="flex items-center gap-4">
                <div className="bg-emerald-50 text-emerald-600 px-4 py-1.5 rounded-full text-[10px] font-black border border-emerald-100 hidden md:block">النظام متصل وآمن</div>
          ...
          <footer className="px-10 py-8 border-t border-slate-50 flex justify-between items-center text-slate-400 font-bold text-[10px] bg-white">
             <span>نظام القمة - النسخة الماسية البلاتينية</span>
