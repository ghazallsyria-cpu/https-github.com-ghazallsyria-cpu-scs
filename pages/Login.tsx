  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] flex flex-col items-center justify-center p-4 font-['Cairo']" dir="rtl">
      ...
      <div className="bg-white w-full max-w-lg p-8 md:p-12 rounded-[3rem] md:rounded-[4rem] shadow-[0_50px_100px_-20px_rgba(0,0,0,0.12)] border border-slate-100 relative overflow-hidden">
        ...
        {/* Identity Tabs - Extremely Clear Switcher */}
        <div className="flex bg-slate-100 p-2 rounded-[2.5rem] mb-12 border border-slate-200">
           <button 
             ...
             className={`flex-1 flex items-center justify-center gap-3 py-5 rounded-[2rem] font-black text-sm transition-all duration-500 ${isParentMode ? 'bg-emerald-600 text-white shadow-xl translate-y-0' : 'text-slate-400 hover:text-emerald-600'}`}
           >
             <UserCircle size={22} /> بوابة ولي الأمر
           </button>
           <button 
             onClick={() => { setIsParentMode(false); setError(null); }}
             className={`flex-1 flex items-center justify-center gap-3 py-5 rounded-[2rem] font-black text-sm transition-all duration-500 ${!isParentMode ? 'bg-indigo-600 text-white shadow-xl translate-y-0' : 'text-slate-400 hover:text-indigo-600'}`}
           >
        ...
        <div className="flex flex-col items-center mb-10 text-center">
           <div className={`p-6 rounded-[2.5rem] ${isParentMode ? 'bg-emerald-50 text-emerald-600' : 'bg-indigo-50 text-indigo-600'} mb-6 transition-colors duration-700 shadow-inner`}>
              {isParentMode ? <School size={54} /> : <GraduationCap size={54} />}
           </div>
        ...
        {isParentMode ? (
          <form onSubmit={handleParentLogin} className="space-y-8 animate-in fade-in duration-500">
            <div className="space-y-3">
               <label className="text-[10px] font-black text-slate-400 mr-6 uppercase tracking-widest">رقم الهاتف المسجل (الطالب/الأب/الأم)</label>
            ...
            <p className="text-center text-[10px] text-slate-400 font-bold leading-relaxed px-10">
              * الدخول برقم الهاتف المسجل لدى المعلم، لا يتطلب كلمة مرور لأولياء الأمور.
            </p>
          </form>
        ) : (
          <form onSubmit={handleTeacherLogin} className="space-y-6 animate-in fade-in duration-500">
            {isSignUp && (
              <div className="space-y-2">
                <label className="text-[10px] font-black text-slate-400 mr-6 uppercase tracking-widest text-right block">الاسم الكامل</label>
            ...
            <div className="space-y-2">
               <label className="text-[10px] font-black text-slate-400 mr-6 uppercase tracking-widest text-right block">رقم الهاتف (اسم المستخدم)</label>
               <div className="relative">
            ...
            <div className="space-y-2">
               <label className="text-[10px] font-black text-slate-400 mr-6 uppercase tracking-widest text-right block">كلمة المرور</label>
               <div className="relative">
