...
import { 
  ...
  ArrowLeft, Download, Database, LayoutGrid, Clock
} from 'lucide-react';
...
const Reports = ({ role, uid, year, semester }: { role: any, uid: string, year: string, semester: string }) => {
  ...
  // دالة لتحويل المصفوفة إلى CSV مع دعم العربية UTF-8 BOM
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      // تطبيق فلترة الوقت على الطلاب عادة ما يكون حسب الفصل/السنة
      ...
      // تنفيذ التصدير بناءً على النوع المختار
      ...
      if (exportType === 'students') {
        ...
        // تقرير شامل (دمج بسيط أو تصدير الطلاب كملخص)
          ...
          <div className="bg-white p-10 rounded-[3.5rem] border border-slate-200 shadow-sm space-y-10">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {[
                  ...
                  <button
                    ...
                    className={`p-6 rounded-[2.5rem] border-2 text-right transition-all flex items-start gap-4 group ${
                      exportType === type.id ? 'border-indigo-600 bg-indigo-50/30' : 'border-slate-50 bg-slate-50/50 hover:border-slate-100'
                    }`}
                  >
              ...
              <div className="flex flex-wrap gap-4">
                 {[
                   ...
                   <button
                    ...
                    className={`flex-1 min-w-[150px] p-5 rounded-3xl border-2 font-black text-xs transition-all flex flex-col items-center gap-3 ${
                      timeRange === range.id ? 'border-indigo-600 bg-indigo-50/50 text-indigo-600' : 'border-slate-50 bg-slate-50 text-slate-400 hover:border-slate-100'
                    }`}
                   >
                     {range.icon}
                     {range.label}
            ...
            <div className="pt-6">
               <button
                ...
                className="w-full py-6 bg-slate-900 hover:bg-indigo-600 text-white rounded-[2.5rem] font-black text-lg shadow-2xl transition-all active:scale-95 flex items-center justify-center gap-4 group disabled:opacity-50"
               >
                 {loading ? (
        ...
        <div className="space-y-8">
           <div className="bg-indigo-600 p-8 rounded-[3.5rem] text-white shadow-2xl relative overflow-hidden group">
              ...
              <h4 className="text-xl font-black mb-6 flex items-center gap-3">
                <FileDown size={24} /> معلومات التصدير
              </h4>
              <ul className="space-y-5">
                {[
                  "يتم ترميز الملفات بـ UTF-8 BOM لدعم اللغة العربية.",
                  "التقرير الشامل يعطيك ملخص المديونيات لجميع الطلاب.",
                  "يمكنك فتح الملفات مباشرة ببرنامج Excel.",
                  "التصدير اليومي يشمل فقط ما تم إنجازه اليوم من حصص أو دفعات."
                ].map((info, i) => (
           ...
           <div className="bg-white p-8 rounded-[3.5rem] border border-slate-200 shadow-sm flex flex-col items-center text-center">
              <div className="bg-emerald-50 text-emerald-600 p-6 rounded-full mb-6 ring-8 ring-emerald-50/50">
                 <CheckCircle size={48} />
              </div>
              <h5 className="font-black text-slate-900 mb-2">البيانات آمنة</h5>
              <p className="text-xs text-slate-400 font-bold leading-relaxed px-4">
                نظام التصدير يقوم فقط بقراءة البيانات ولا يقوم بتعديلها أو حذفها، مما يضمن سلامة معلوماتك دائماً.
              </p>
           </div>
        </div>
      </div>
