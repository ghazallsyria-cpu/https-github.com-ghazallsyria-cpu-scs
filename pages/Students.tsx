...
const Students: React.FC = () => {
  ...
  const handleAddStudent = async (e: React.FormEvent) => {
    ...
    try {
      ...
      fetchStudents();
    } catch (err) { alert('خطأ في إضافة الطالب. تأكد من جدول students'); }
  };
  ...
  const handleAddLesson = async (e: React.FormEvent) => {
    ...
    } catch (err) { alert('خطأ! هل قمت بإنشاء جدول lessons في SQL؟'); }
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        ...
        <button onClick={() => setIsModalOpen(true)} className="bg-indigo-600 text-white px-6 py-2 rounded-xl flex items-center gap-2 hover:bg-indigo-700 shadow-md">
      ...
      <div className="relative">
        ...
        <input 
          ...
          className="w-full pr-10 pl-4 py-3 bg-white border border-slate-200 rounded-xl focus:ring-2 focus:ring-indigo-500 outline-none shadow-sm"
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {loading ? (
          <p className="col-span-full text-center py-10 font-bold text-slate-500">جاري تحميل البيانات...</p>
          ...
          filteredStudents.map(student => (
            <div key={student.id} className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm hover:shadow-md transition-shadow">
              <div className="flex justify-between items-start mb-4">
                ...
                <button onClick={() => { setSelectedStudent(student); setIsLessonModalOpen(true); }} className="bg-indigo-600 text-white p-2 rounded-lg hover:bg-indigo-700 transition-colors flex items-center gap-1 text-xs font-bold">
              ...
              <div className="space-y-2 mb-6 text-sm text-slate-600">
                <div className="flex items-center gap-2"><MapPin size={16} className="text-slate-400"/> <span>{student.address}</span></div>
                <div className="flex items-center gap-2"><Phone size={16} className="text-slate-400"/> <span>{student.phone}</span></div>
              </div>
          ...
          <div className="col-span-full text-center py-20 bg-slate-50 rounded-3xl border-2 border-dashed border-slate-200">
            ...
            <p className="text-slate-500 font-bold">لا يوجد طلاب مسجلين حالياً.</p>
          </div>
        )}
      </div>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div className="bg-white w-full max-w-md rounded-3xl p-8 relative animate-in zoom-in duration-200">
            <h2 className="text-2xl font-black mb-6 text-slate-900">إضافة طالب جديد</h2>
            <form onSubmit={handleAddStudent} className="space-y-4">
              <div>
                <label className="block text-sm font-bold mb-1 mr-1">الاسم الكامل</label>
                <input required className="w-full p-3 border rounded-xl outline-none focus:ring-2 focus:ring-indigo-500" value={studentForm.name} onChange={e => setStudentForm({...studentForm, name: e.target.value})} />
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-bold mb-1 mr-1">الصف/المستوى</label>
                  <input required className="w-full p-3 border rounded-xl outline-none" value={studentForm.grade} onChange={e => setStudentForm({...studentForm, grade: e.target.value})} />
                </div>
                <div>
                  <label className="block text-sm font-bold mb-1 mr-1">المبلغ المتفق عليه</label>
                  <input required type="number" className="w-full p-3 border rounded-xl outline-none" value={studentForm.agreed_payment} onChange={e => setStudentForm({...studentForm, agreed_payment: e.target.value})} />
                </div>
              </div>
              <div>
                <label className="block text-sm font-bold mb-1 mr-1">رقم الهاتف</label>
                <input required className="w-full p-3 border rounded-xl outline-none" value={studentForm.phone} onChange={e => setStudentForm({...studentForm, phone: e.target.value})} />
              </div>
              <div>
                <label className="block text-sm font-bold mb-1 mr-1">العنوان</label>
                <textarea required className="w-full p-3 border rounded-xl outline-none h-24" value={studentForm.address} onChange={e => setStudentForm({...studentForm, address: e.target.value})} />
              </div>
              <div className="flex gap-3 pt-4">
                <button type="button" onClick={() => setIsModalOpen(false)} className="flex-1 py-3 border border-slate-200 rounded-xl font-bold hover:bg-slate-50">إلغاء</button>
                <button type="submit" className="flex-1 py-3 bg-indigo-600 text-white rounded-xl font-bold hover:bg-indigo-700 shadow-lg shadow-indigo-100">حفظ الطالب</button>
              </div>
            </form>
          </div>
        </div>
      ...
      {isLessonModalOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div className="bg-white w-full max-w-md rounded-3xl p-8 relative animate-in zoom-in duration-200">
            <h2 className="text-2xl font-black mb-2 text-slate-900">تسجيل حصة جديدة</h2>
            ...
            <form onSubmit={handleAddLesson} className="space-y-5">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input required type="date" className="w-full p-3 border rounded-xl outline-none focus:ring-2 focus:ring-indigo-500" value={lessonForm.date} onChange={e => setLessonForm({...lessonForm, date: e.target.value})} />
                </div>
                <div>
                  <label className="block text-sm font-bold mb-1 mr-1">عدد الساعات</label>
                  <input required type="number" step="0.5" placeholder="مثال: 1.5" className="w-full p-3 border rounded-xl outline-none" value={lessonForm.hours} onChange={e => setLessonForm({...lessonForm, hours: e.target.value})} />
                </div>
              </div>
              <div>
                <label className="block text-sm font-bold mb-1 mr-1">ملاحظات الدرس</label>
                <textarea placeholder="ماذا تم إنجازه في هذه الحصة؟" className="w-full p-3 border rounded-xl outline-none h-32" value={lessonForm.notes} onChange={e => setLessonForm({...lessonForm, notes: e.target.value})} />
              </div>
