...
import { 
  ...
  Sparkles, UserCheck, Link as LinkIcon, Briefcase
} from 'lucide-react';
...
const Students = ({ isAdmin, role, uid, year, semester }: { isAdmin: boolean, role: any, uid: string, year: string, semester: string }) => {
  ...
  const [opsConfig, setOpsConfig] = useState({ targetYear: year, targetSemester: semester, targetGrade: '', includeLessons: false, includePayments: false, operationType: 'copy' as 'copy' | 'move' });
  ...
  const fetchStudents = useCallback(async () => {
    ...
    try {
      ...
      // منطق ذكي للمدير: تجميع الطلاب المتكررين في سجل واحد ظاهرياً
      if (role === 'admin') {
        const unified: any[] = [];
        const seenPhones = new Set();
        data?.forEach(s => {
          const phone = s.phones?.[0]?.number;
  ...
  // فحص وجود الطالب عند كتابة رقم الهاتف (الربط الذكي)
  const checkExistingStudent = async (phone: string) => {
    if (phone.length < 8) return;
    const { data } = await supabase
      .from('students')
      .select('*, profiles(full_name)')
      .contains('phones', [{ number: phone }])
      .limit(1);
    
    if (data && data.length > 0) {
      setExistingStudentFound(data[0]);
      // تعبئة تلقائية للبيانات إذا كان طالباً جديداً
      if (!isEditMode) {
        setForm(prev => ({
          ...prev,
          name: data[0].name,
  ...
  return (
    <div className="space-y-12 animate-in fade-in duration-700 text-right font-['Cairo'] pb-32">
      {feedback && (
        <div className={`fixed top-8 left-1/2 -translate-x-1/2 z-[500] px-12 py-6 rounded-[2.5rem] shadow-2xl flex items-center gap-5 font-black transition-all animate-in slide-in-from-top-4 ${feedback.type === 'success' ? 'bg-emerald-600 text-white' : 'bg-rose-600 text-white'}`}>
      ...
      <div className="bg-white p-12 lg:p-16 rounded-[5rem] border border-slate-100 shadow-xl flex flex-col md:flex-row justify-between items-center gap-10 relative overflow-hidden">
        ...
        <div className="flex items-center gap-10 z-10">
           <div className="bg-indigo-600 p-8 rounded-[2.8rem] text-white shadow-2xl shadow-indigo-100 rotate-6 group-hover:rotate-0 transition-all duration-700"><Users size={44}/></div>
           <div>
              <h1 className="text-4xl font-black text-slate-900 leading-tight">سجل الطلاب {isAdmin ? '(هوية موحدة)' : ''}</h1>
              <p className="text-[11px] font-black text-slate-400 uppercase tracking-[0.3em] mt-3">إدارة {students.length} سجلات تعليمية ذكية</p>
           </div>
        </div>
      ...
      <div className="flex gap-5 overflow-x-auto no-scrollbar pb-4">
        {['الكل', '10', '11', '12'].map(grade => (
          <button key={grade} onClick={() => setSelectedGrade(grade)} className={`px-14 py-5 rounded-[2rem] font-black text-sm transition-all border-2 ${selectedGrade === grade ? 'bg-indigo-600 text-white shadow-2xl border-indigo-600 translate-y-[-4px]' : 'bg-white text-slate-400 border-slate-100 hover:border-indigo-100'}`}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
        {loading ? (
          <div className="col-span-full py-40 flex flex-col items-center gap-6"><RefreshCw className="animate-spin text-indigo-600" size={60} /><p className="font-black text-slate-300 uppercase tracking-widest">مزامنة الهويات الرقمية...</p></div>
          ...
          <div key={s.id} className="bg-white p-10 rounded-[4.5rem] border border-slate-100 shadow-sm hover:shadow-2xl transition-all group flex flex-col relative overflow-hidden">
            <div className="flex justify-between items-start mb-8 relative z-10">
               <div>
                 <h3 className="text-2xl font-black text-slate-900 group-hover:text-indigo-600 transition-colors truncate max-w-[200px]">{s.name}</h3>
                 <div className="flex flex-wrap gap-2 mt-3">
                    ...
                    {isAdmin && s.all_teachers && (
                       <div className="flex items-center gap-2 bg-emerald-50 text-emerald-600 px-5 py-1.5 rounded-full text-[9px] font-black border border-emerald-100">
                          <Briefcase size={10}/> {s.all_teachers.length} مدرسين
                       </div>
                    )}
                 </div>
               </div>
               <div className="flex gap-3">
                 ...
                 <button onClick={() => { if(confirm('حذف السجل؟')) supabase.from('students').delete().eq('id', s.id).then(() => fetchStudents()); }} className="p-4 bg-rose-50 text-rose-400 rounded-2xl hover:bg-rose-600 hover:text-white transition-all shadow-sm"><Trash2 size={22}/></button>
               </div>
            </div>

            {/* عرض المعلمين المرتبطين في حال الإدارة */}
            {isAdmin && s.all_teachers && s.all_teachers.length > 1 && (
               <div className="mb-6 p-4 bg-slate-50 rounded-2xl border border-slate-100">
                  <p className="text-[9px] font-black text-slate-400 uppercase mb-2">المدرسون المشتركون:</p>
                  <div className="flex flex-wrap gap-2">
                     {s.all_teachers.map((t: string, idx: number) => (
                        <span key={idx} className="bg-white px-3 py-1 rounded-lg text-[9px] font-bold text-slate-600 shadow-sm">{t}</span>
                     ))}
                  </div>
               </div>
            )}
            ...
            <div className="pt-8 border-t border-slate-50 flex justify-between items-center">
               <div>
                  <p className="text-[10px] font-black text-slate-300 uppercase tracking-widest mb-1">المستحق المالي {isAdmin ? '(كلي)' : ''}</p>
        ...
        <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-3xl z-[600] flex items-center justify-center p-6">
          <form onSubmit={handleSaveStudent} className="bg-white w-full max-w-3xl p-14 lg:p-20 rounded-[5rem] shadow-2xl relative animate-in zoom-in duration-500 max-h-[95vh] overflow-y-auto no-scrollbar border border-white/20">
            ...
            <h2 className="text-4xl font-black mb-16 text-slate-900 flex items-center gap-6">
              ...
              {isEditMode ? 'تحديث بيانات الطالب' : 'تسجيل طالب جديد'}
            </h2>
            ...
            <div className="space-y-10">
               ...
               {existingStudentFound && !isEditMode && (
                  <div className="bg-emerald-50 p-8 rounded-[2.5rem] border-2 border-emerald-100 animate-in bounce-in flex items-center gap-6">
                     ...
                     <div>
                        ...
                        <p className="text-xs font-bold text-emerald-700 mt-1">مسجل لدى الأستاذ: {existingStudentFound.profiles?.full_name}. سيتم ربط السجل الجديد بنفس الهوية.</p>
                     </div>
                  </div>
               )}
               ...
               <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  <div className="space-y-3">
                    <label className="text-[11px] font-black text-slate-400 mr-6 uppercase tracking-widest">رقم الهاتف (للمطابقة الذكية)</label>
                    <div className="relative">
                       ...
                       <input 
                         ...
                         className="w-full pr-16 pl-8 py-6 bg-slate-50 border-none rounded-[2.2rem] font-black text-2xl text-left tracking-widest outline-none focus:bg-white focus:ring-4 focus:ring-indigo-50 transition-all shadow-inner" 
                         value={form.phones[0].number} 
                         onChange={e => {
                           const n = [...form.phones]; 
                           n[0].number = e.target.value; 
                           setForm({...form, phones: n});
                  ...
                  <div className="space-y-3">
                    <label className="text-[11px] font-black text-slate-400 mr-6 uppercase tracking-widest">الاسم المعتمد</label>
               ...
               <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  ...
                  <div className="space-y-3">
                    <label className="text-[11px] font-black text-slate-400 mr-6 uppercase tracking-widest">اسم المدرسة (اختياري)</label>
                    <input placeholder="المدرسة..." className="w-full p-6 bg-slate-50 border-none rounded-[2.2rem] font-black text-lg outline-none focus:bg-white focus:ring-4 focus:ring-indigo-50 transition-all shadow-inner" value={form.school_name} onChange={e => setForm({...form, school_name: e.target.value})} />
                  </div>
               </div>
               ...
               <div className="p-12 bg-indigo-50/50 rounded-[4rem] space-y-8 border-4 border-dashed border-indigo-100">
                  <div className="flex items-center gap-6">
                    ...
                    <label htmlFor="h" className="font-black text-slate-700 text-lg cursor-pointer">محاسبة بنظام الساعة (خارجي)</label>
                  </div>
                  {form.is_hourly ? (
                    ...
                    <input type="number" placeholder="المبلغ الفصلي المقطوع ($)" className="w-full p-8 bg-white rounded-[2.5rem] font-black text-center text-5xl text-slate-900 shadow-2xl outline-none" value={form.agreed_amount} onChange={e => setForm({...form, agreed_amount: e.target.value})} />
                  )}
               </div>
            </div>
            ...
            <button disabled={isSubmitting} className="w-full py-8 bg-indigo-600 text-white font-black rounded-[3rem] mt-16 shadow-2xl hover:bg-indigo-700 transition-all flex items-center justify-center gap-5 active:scale-95 text-2xl group">
              ...
              تأكيد البيانات {existingStudentFound ? 'وربط الهوية' : 'والحفظ'}
            </button>
          </form>
        </div>
      )}

      {/* MODAL: OPS (ترحيل/نقل) - نتركه كما هو للعمليات المتقدمة */}
      {isOpsModalOpen && studentToManage && (
         <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-3xl z-[500] flex items-center justify-center p-6">
             <div className="bg-white w-full max-w-2xl p-12 lg:p-16 rounded-[5rem] shadow-2xl relative animate-in zoom-in duration-500 overflow-y-auto max-h-[95vh] no-scrollbar">
                <button onClick={() => setIsOpsModalOpen(false)} className="absolute top-12 left-12 text-slate-300 hover:text-rose-500 transition-all"><X size={44}/></button>
                <div className="text-center mb-14">
                   <div className="bg-indigo-600 p-8 rounded-3xl text-white inline-block shadow-2xl mb-6"><ArrowRightLeft size={44}/></div>
                   <h2 className="text-4xl font-black text-slate-900">عمليات الترحيل والتحكم</h2>
                   <p className="text-indigo-500 font-black mt-4 uppercase tracking-widest">الطالب: {studentToManage.name}</p>
                </div>
