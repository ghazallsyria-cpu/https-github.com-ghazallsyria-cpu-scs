...
import { 
  ...
  Settings, UserCheck, Link as LinkIcon, Briefcase, X
} from 'lucide-react';
    ...
    <div className="space-y-12 animate-in fade-in duration-700 text-right font-['Cairo'] pb-32">
      ...
      <div className="bg-white p-12 lg:p-16 rounded-[5rem] border border-slate-100 shadow-xl flex flex-col md:flex-row justify-between items-center gap-10 relative overflow-hidden">
        ...
        <div className="flex items-center gap-10 z-10">
           ...
           <div>
              <h1 className="text-4xl font-black text-slate-900 leading-tight">سجل الطلاب</h1>
              <p className="text-[11px] font-black text-slate-400 uppercase tracking-[0.3em] mt-3">إدارة {students.length} هوية طلابية</p>
           </div>
        </div>
          ...
          <div key={s.id} className="bg-white p-10 rounded-[4.5rem] border border-slate-100 shadow-sm hover:shadow-2xl transition-all group flex flex-col relative overflow-hidden">
            <div className="flex justify-between items-start mb-8 relative z-10">
               <div>
                 ...
                 <span className="bg-indigo-50 text-indigo-600 px-5 py-1.5 rounded-full text-[9px] font-black border border-indigo-100 mt-2 block w-fit">الصف {s.grade}</span>
               </div>
            ...
            <div className="pt-8 border-t border-slate-50 flex justify-between items-center">
               <div>
                  <p className="text-[10px] font-black text-slate-300 uppercase tracking-widest mb-1">المتبقي المالي</p>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-3xl z-[600] flex items-center justify-center p-6">
          <form onSubmit={handleSaveStudent} className="bg-white w-full max-w-3xl p-14 lg:p-20 rounded-[5rem] shadow-2xl relative animate-in zoom-in duration-500 max-h-[95vh] overflow-y-auto no-scrollbar border border-white/20">
            {/* Added X to imports and used here */}
            ...
            <h2 className="text-4xl font-black mb-16 text-slate-900">بيانات الطالب</h2>
            <div className="space-y-10">
               <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  <input required placeholder="الاسم الكامل" className="w-full p-6 bg-slate-50 border-none rounded-[2.2rem] font-black text-xl outline-none" value={form.name} onChange={e => setForm({...form, name: e.target.value})} />
                  <input required type="tel" placeholder="رقم الهاتف" className="w-full p-6 bg-slate-50 border-none rounded-[2.2rem] font-black text-xl text-left outline-none" value={form.phones[0].number} onChange={e => {
               ...
               <div className="p-12 bg-indigo-50/50 rounded-[4rem] space-y-8 border-4 border-dashed border-indigo-100">
                  <div className="flex items-center gap-6">
                    <input type="checkbox" id="h" checked={form.is_hourly} onChange={e => setForm({...form, is_hourly: e.target.checked})} className="w-8 h-8 accent-indigo-600" />
                    <label htmlFor="h" className="font-black text-slate-700 text-lg">محاسبة بنظام الساعة</label>
                  </div>
                  {form.is_hourly ? (
                    <input type="number" placeholder="سعر الساعة ($)" className="w-full p-8 bg-white rounded-[2.5rem] font-black text-center text-5xl text-indigo-600 outline-none" value={form.price_per_hour} onChange={e => setForm({...form, price_per_hour: e.target.value})} />
                  ) : (
                    <input type="number" placeholder="مبلغ المادة ($)" className="w-full p-8 bg-white rounded-[2.5rem] font-black text-center text-5xl text-slate-900 outline-none" value={form.agreed_amount} onChange={e => setForm({...form, agreed_amount: e.target.value})} />
                  )}
               </div>
               <button type="submit" className="w-full py-8 bg-indigo-600 text-white font-black rounded-[3rem] text-2xl shadow-xl">حفظ البيانات</button>
            </div>
          </form>
        </div>
      )}
