...
const Students = ({ role, uid }: { role: any, uid: string }) => {
  ...
  const fetchStudents = useCallback(async () => {
    setLoading(true);
    try {
      // استعلام بسيط للطلاب مع جلب بيانات البروفايل (المعلم)
      ...
      // جلب الدروس بشكل منفصل لتجنب تعقيد الاستعلام وتقليل احتمالية خطأ RLS
      const { data: lsns, error: lErr } = await supabase.from('lessons').select('*');
      if (lErr) throw lErr;

      const enriched = (stds || []).map(s => {
        const sLsn = (lsns || []).filter(l => l.student_id === s.id);
        // معالجة بيانات البروفايل لتكون متوافقة سواء كانت كائن أو مصفوفة
      ...
      showFeedback(e.message || "فشل في جلب البيانات", "error");
    } finally {
      setLoading(false);
    }
  }, [uid, role, isAdmin]);
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      {feedback && (
        <div className={`fixed top-6 left-1/2 -translate-x-1/2 z-[150] px-8 py-4 rounded-2xl shadow-2xl flex items-center gap-3 font-bold ${feedback.type === 'success' ? 'bg-emerald-600' : 'bg-rose-600'} text-white`}>
      ...
      <div className="flex flex-col md:flex-row justify-between items-center gap-6">
        <div>
          <h1 className="text-4xl font-black text-slate-900">إدارة الطلاب</h1>
          <p className="text-slate-500 font-bold mt-2">
            {isAdmin ? "وضع الإدارة العامة: عرض كافة المعلمين" : `أهلاً بك، لديك ${students.length} طالب.`}
          </p>
        </div>
        <div className="flex flex-wrap gap-4">
          <div className="relative">
            ...
            <input 
              ...
              className="pr-12 pl-4 py-4 bg-white border border-slate-200 rounded-2xl outline-none focus:ring-2 focus:ring-indigo-500 w-64"
              value={searchTerm}
          ...
          <button 
            ...
            className="bg-indigo-600 text-white px-8 py-4 rounded-2xl font-black shadow-xl"
          >
            إضافة طالب
          </button>
        </div>
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {filteredStudents.map(s => (
            <div key={s.id} className="bg-white p-8 rounded-[2.5rem] border border-slate-200 shadow-sm relative hover:shadow-lg transition-all">
              {isAdmin && s.profiles && (
      ...
      {/* مودال الإضافة وتفاصيل الدروس تبقى كما هي تقريباً مع التأكد من إغلاقها */}
      {isModalOpen && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[100] flex items-center justify-center p-4">
          <form onSubmit={handleAddStudent} className="bg-white w-full max-w-lg p-10 rounded-[3rem] shadow-2xl relative animate-in zoom-in duration-300">
            ...
            <div className="grid grid-cols-2 gap-4">
              <input required placeholder="الاسم" className="p-4 bg-slate-50 rounded-2xl" value={form.name} onChange={e => setForm({...form, name: e.target.value})} />
              <input required placeholder="الصف" className="p-4 bg-slate-50 rounded-2xl" value={form.grade} onChange={e => setForm({...form, grade: e.target.value})} />
              <input required placeholder="الهاتف" className="p-4 bg-slate-50 rounded-2xl" value={form.phone} onChange={e => setForm({...form, phone: e.target.value})} />
              <input required type="number" placeholder="الاتفاق ($)" className="p-4 bg-slate-50 rounded-2xl" value={form.agreed_amount} onChange={e => setForm({...form, agreed_amount: e.target.value})} />
              <input required placeholder="العنوان" className="p-4 bg-slate-50 rounded-2xl col-span-2" value={form.address} onChange={e => setForm({...form, address: e.target.value})} />
            </div>
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[100] flex items-center justify-center p-4">
          <form onSubmit={handleAddLesson} className="bg-white w-full max-w-md p-10 rounded-[3rem] shadow-2xl relative animate-in zoom-in duration-300">
            ...
            <div className="space-y-4">
              <input required type="date" className="w-full p-4 bg-slate-50 rounded-2xl" value={lessonForm.lesson_date} onChange={e => setLessonForm({...lessonForm, lesson_date: e.target.value})} />
              <input required type="number" step="0.5" placeholder="عدد الساعات" className="w-full p-4 bg-slate-50 rounded-2xl" value={lessonForm.hours} onChange={e => setLessonForm({...lessonForm, hours: e.target.value})} />
              <textarea placeholder="ملاحظات" className="w-full p-4 bg-slate-50 rounded-2xl" value={lessonForm.notes} onChange={e => setLessonForm({...lessonForm, notes: e.target.value})} />
