...
import { Plus, Trash2, CheckCircle, X, AlertCircle, Users, School, MessageCircle, Phone, MapPin, Search, Folder, FolderOpen, Layers } from 'lucide-react';
...
const Students = ({ role, uid, year, semester }: { role: any, uid: string, year: string, semester: string }) => {
  ...
  // تصفية الطلاب بناءً على المجلد المختار والبحث
        ...
        <div className="flex gap-4 w-full md:w-auto">
          <div className="relative flex-1 md:w-64">
            ...
            <input 
              ...
              className="w-full pr-10 pl-4 py-3 bg-slate-50 border rounded-xl text-sm font-bold focus:bg-white transition-all"
              value={searchTerm}
        ...
        {[
          ...
          return (
            ...
            >
              ...
              {isActive && <div className="absolute top-2 right-2 w-2 h-2 bg-indigo-600 rounded-full animate-ping"></div>}
            </button>
          );
        })}
      </div>
            ...
            <div className="space-y-5">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-1">
                  ...
                  <input required placeholder="محمد أحمد..." className="w-full p-4 bg-slate-50 border rounded-2xl font-bold" value={form.name} onChange={e => setForm({...form, name: e.target.value})} />
                </div>
                <div className="space-y-1">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-1">
                  ...
                  <input placeholder="اسم المدرسة" className="w-full p-4 bg-slate-50 border rounded-2xl font-bold" value={form.school_name} onChange={e => setForm({...form, school_name: e.target.value})} />
                </div>
                <div className="space-y-1">
                  <label className="text-[10px] font-black text-slate-400 mr-2 uppercase">العنوان / السكن</label>
                  <input placeholder="المنطقة - الشارع" className="w-full p-4 bg-slate-50 border rounded-2xl font-bold" value={form.address} onChange={e => setForm({...form, address: e.target.value})} />
                </div>
              </div>
              ...
              <div className="bg-slate-50 p-6 rounded-3xl border border-slate-100">
                {form.is_hourly ? (
                  <input required type="number" placeholder="سعر الساعة الدراسية ($)" className="w-full p-4 bg-white border rounded-2xl font-black" value={form.price_per_hour} onChange={e => setForm({...form, price_per_hour: e.target.value})} />
                ) : (
                  <input required type="number" placeholder="المبلغ الفصلي الإجمالي ($)" className="w-full p-4 bg-white border rounded-2xl font-black" value={form.agreed_amount} onChange={e => setForm({...form, agreed_amount: e.target.value})} />
                )}
              </div>
            </div>
            <button type="submit" className="w-full py-5 bg-indigo-600 text-white font-black rounded-2xl mt-8 shadow-xl hover:bg-indigo-700 transition-all">حفظ وحفظ الطالب</button>
          </form>
        </div>
      )}
    </div>
